...
namespace Camelot.forms
{
	...
	/// </summary>
	public class contactcorrespondence : System.Web.UI.Page
	{
		...
		private Camelot.classes.cContact pCont;
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			pCont = (Camelot.classes.cContact)Session["curContact"];

			if (!Page.IsClientScriptBlockRegistered("send"))
			{		
				string title="Contact Correspondence";
				string height="700";
				string width="800";	
				string cont = pCont.Contact_ID;
			 
				// use next line for direct with <base target="_self"> between <Head> and </HEAD> 
				string scrp="<script>var rc = new Array(0,0);function doRIt(){rc= window.showModalDialog('contcorr.aspx?Title="+title + "&cont=" + cont + "','','dialogHeight:"+height+" px;dialogWidth:"+width+" px;');"+"\n";
			...
			else
			{
				...
				bool vis = false;
				getCorrs();
				this.lblCont.Text = pCont.Contact_Name;
			}
		}
		...
		private void getCorrs()
		{
			...
			cmd.CommandText = "get_Contact_corr";

			cmd.Parameters.Add(
				new SqlParameter("@cont_id", pCont.Contact_ID));
		...
		public void dgCorrs_ItemDataBound(Object sender, DataGridItemEventArgs e)
		{
			if(e.Item.ItemType != ListItemType.Header && e.Item.ItemType != ListItemType.Footer)
			{
				...
				{
					fButton.Attributes.Add("onclick", "javascript:window.showModalDialog('corrdetail.aspx?corr=" + DataBinder.Eval(e.Item.DataItem, "corr_id") + "','contdets','dialogHeight: 550px; dialogWidth: 800px; dialogTop: 100px; dialogLeft:100px; edge: Raised; center: No; help: No; resizable: No; status: No;');");
				}
			}
		}
