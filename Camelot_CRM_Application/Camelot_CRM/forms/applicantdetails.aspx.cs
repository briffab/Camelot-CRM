...
namespace Camelot.forms
{
	...
	/// </summary>
	public class applicantdetails : System.Web.UI.Page
	{
		...
		protected System.Web.UI.WebControls.TextBox txtPet;
		protected System.Web.UI.WebControls.Label Label23;
		protected System.Web.UI.WebControls.TextBox txtOccup;
		protected System.Web.UI.WebControls.TextBox txtEmp;
		protected System.Web.UI.WebControls.Label Label24;
		protected System.Web.UI.WebControls.TextBox txtSin;
		protected System.Web.UI.WebControls.Label Label25;
		protected System.Web.UI.WebControls.TextBox txtGin;
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!Page.IsClientScriptBlockRegistered("Place"))
			{		
				string title="Place Guardian";
				string height="250";
				string width="500";	
			 
				// use next line for direct with <base target="_self"> between <Head> and </HEAD> 
				string scrp="<script>var rc = new Array(0,0);function doIt(){if(Form1.txtLname.value + Form1.txtFname.value != ''){rc= window.showModalDialog('placeGuard.aspx?Title="+title + "','','dialogHeight:"+height+" px;dialogWidth:"+width+" px;');"+"\n";
				scrp+="__doPostBack('','');}else{alert('Please supply a name for the Guardian');}}</script>";

				Page.RegisterStartupScript("Place",scrp);			
				this.btnPlace.Attributes.Add("onClick","doIt();");
			}
		...
		private void pop_app(string app_id)
		{
			...
			while(rdr.Read())
			{
				...
				this.txtInit.Text = rdr["init"].ToString();
				...
				this.txtSin.Text = rdr["sincome"].ToString();
				trans = rdr["trans"].ToString();
				pets = rdr["pets"].ToString();
				this.txtGin.Text = rdr["gincome"].ToString();
				this.txtPet.Text = rdr["animal"].ToString();
				this.txtEmp.Text = rdr["employer"].ToString();
				...
				this.chkED.Checked = Convert.ToBoolean(rdr["exec_des"]);
		...
		private void pop_combos(string gen, string trans, string pets)
		{
			int ind = 0;
			int cnt = 0;
		...
		private void savedetails()
		{
			...
			if(oktosave)
			{
				...
				cmd.Parameters.Add(
					new SqlParameter("@init", this.txtInit.Text));
				cmd.Parameters.Add(
				...
				cmd.Parameters.Add(
					new SqlParameter("@sin", this.txtSin.Text));
				cmd.Parameters.Add(
					new SqlParameter("@trans", this.cmbTrans.SelectedValue));
				cmd.Parameters.Add(
					new SqlParameter("@pets", this.cmbPets.SelectedValue));
				cmd.Parameters.Add(
					new SqlParameter("@gin", this.txtGin.Text));
				cmd.Parameters.Add(
					new SqlParameter("@pet", this.txtPet.Text));
				cmd.Parameters.Add(
					new SqlParameter("@emp", this.txtEmp.Text));
				cmd.Parameters.Add(
		...
		private void setenabled(bool edit)
		{
			...
			this.txtPet.Enabled = edit;
			this.txtOccup.Enabled = edit;
			this.txtEmp.Enabled = edit;
			this.txtSin.Enabled = edit;
			this.txtGin.Enabled = edit;
		...
		private void set_app()
		{
			...
			app.Sin = this.txtSin.Text;
			app.Trans = this.cmbTrans.SelectedValue;
			app.Pets = this.cmbPets.SelectedValue;
			app.Gin = this.txtGin.Text;
			app.Pet = this.txtPet.Text;
			app.Emp = this.txtEmp.Text;
			...
			app.Exec = Convert.ToInt32(this.chkED.Checked);
